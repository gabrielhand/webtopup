...
const Services = () => {
  ...
  return (
    <div className="flex flex-col lg:gap-y-6 md:gap-y-6 gap-y-4 lg:px-16 md:px-10 px-4 py-6">
      ...
      <div className="relative flex flex-row w-full justify-between -my-2">
        ...
        >
          {kategoris.map((kategori) => (
            ...
            >
              <p className="group-hover:text-white text-black dark:text-white group-hover:duration-150">{kategori.nama}</p>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
                height="20"
                fill="currentColor"
                className={`bi bi-check-lg ${ selectedKategori === kategori.nama ? "block text-[#4169e1] group-hover:text-white group-hover:duration-150" : "hidden"}`}
                viewBox="0 0 16 16"
              >
      ...
      <AllServices searchLayanan={searchLayanan} selectedKategori={selectedKategori} />
    </div>
  );
};
